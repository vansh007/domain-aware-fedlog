...
memory-free EWC alternative, which we compare directly. We distill the whole map into a deployable,
summary-statistics-only safety auditor that flags each failure and prescribes its fix \emph{before}
training. Every reported number comes from a real run on commodity CPU.
\end{abstract}
...
head-to-head---a memory-based replay buffer and memory-free EWC---that each remove the forgetting;
(iv) a deployable \emph{safety auditor} that operationalizes the map: from per-client summary
statistics alone (event-id blocks and normal-length ranges, never raw logs) it predicts which of
these failures a planned federation will hit and prescribes the fix, released as a command-line
tool; and (v) a CPU-only research artifact in which every reported number traces to a result file.
...
\textbf{A deployable safety auditor.} These three guidances are decision rules over quantities a
