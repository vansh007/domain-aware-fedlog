...
representation\}, in which exactly one cell fails---together with simple, effective fixes for both
failures that do occur: a domain-aware aggregation that restores the collapsed range method (and
routes with no oracle domain tag, at $100\%$ inferred-routing accuracy), and a small replay buffer
($\approx10\%$ of the earlier domain's data) that cuts the forgetting from $0.665$ to $0.004$ while
still learning the new domain. All results are reproduced from real runs on commodity CPU.
\end{abstract}
...
arrival is catastrophic \emph{iff} the representation shares a feature space, together with a replay
mitigation that removes the forgetting at negligible cost; and (iv) a reproducible, CPU-only artifact
...
A real deployment may not provide one, so we replace the oracle tag with an \emph{inferred} tag:
because each domain owns a contiguous, disjoint id block ($[0,33)$ for HDFS, $[33,427)$ for BGL), a
sequence's domain is the majority block of its event ids. We evaluate this router directly in
Section~\ref{sec:h1}: it routes every test sequence correctly and reproduces the oracle-tag $F_1$
exactly, so the fix is deployable without any external domain label.
...
\textbf{The fix needs no oracle tag.} To show the fix is deployable rather than dependent on a
ground-truth domain label, we replace the oracle tag with the inferred router of
Section~\ref{sec:da}, which reads each sequence's domain off its event-id block. Across five seeds
the router assigns the correct domain to \emph{every} test sequence (routing accuracy
$1.000\pm0.000$ on all $569{,}479$ HDFS and $68{,}874$ BGL test sequences) and therefore reproduces
the oracle-tag $F_1$ exactly (HDFS $0.5614\pm0.0236$; \texttt{results/h1\_inferred\_router.csv}).
The disjoint id blocks that \emph{cause} the domain-blind collapse are the same structure that makes
the domain recoverable at inference, so no external label is required.
...
input\}: only \emph{(sequential, shared representation)} fails, and it fails hard. Simultaneous
...
Table~\ref{tab:replay} reports the result over three seeds. Without replay the embedding model
forgets HDFS ($F_1$ $0.70\!\rightarrow\!0.04$, forgetting $0.665\pm0.003$, reproducing
...
budget a small replay buffer---and leaves parameter-based defenses (EWC~\cite{kirkpatrick2017overcoming})
as a memory-free alternative to compare in future work.
...
no federation, up to $100$ passes; \texttt{results/deeplog\_centralized\_hdfs.csv}). It likewise does
not approach $0.93$: training loss plateaus at $\approx0.204$ by round $20$ and the transductive $F_1$
peaks at $0.43$ (round $2$) before settling near $0.25$, with precision saturating at $\approx0.97$
while recall stays below $0.30$. This confirms the ceiling is a property of the scalar-input
...
failure depends jointly on the detection method, the arrival pattern, and the input representation,
together with working fixes for both failing cases: a domain-aware aggregation that routes without an
oracle tag, and a small replay buffer that removes the forgetting. Future work includes (i) a
memory-free anti-forgetting defense (EWC~\cite{kirkpatrick2017overcoming}) compared against the replay
buffer under a fixed storage budget; (ii) longer arrival sequences ($>2$ domains) where replay-buffer
sizing and interference become harder; and (iii) additional domains to stress the map further.
