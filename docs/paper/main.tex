...
Transformer both forget---together with mitigations (replay and EWC) that each remove it and that we
benchmark against the standard episodic-memory method A-GEM, over which simple replay is
Pareto-competitive; (iv) a generalization beyond the pair to a \emph{three-domain} federation (adding OpenStack) that
...
failures a planned federation will hit and prescribes the fix---released as a tool and \emph{validated}
as a predictor, correct on $14$ of $15$ measured configurations; and (vi) a CPU-only artifact in which
every reported number traces to a result file.
...
\subsection{Comparison to an established method}\label{sec:baselines}
Replay is deliberately the simplest possible defense, so the fair question is whether a purpose-built
continual-learning method does better. We compare against A-GEM~\cite{agem2019}, the standard
episodic-memory method, at the \emph{same} $10\%$ memory budget: instead of training on the buffer
jointly, A-GEM projects each new-domain gradient to remove any component that would increase loss on a
memory batch of the earlier domain. Table~\ref{tab:baselines} places the defenses side by side. All
three remove the forgetting (HDFS retention $\approx0.69$--$0.70$ versus $0.04$ undefended). At the
matched budget our plain replay is at least as good as A-GEM on retention and forgetting
($+0.004$ vs.\ $+0.006$) and---because A-GEM's projection constrains the new-domain update---learns BGL
\emph{better} ($0.722$ vs.\ $0.657$); the memory-free EWC learns BGL best but needs the
six-order-of-magnitude penalty search of Section~\ref{sec:ewc} and forgets slightly more. The simple
fix is thus Pareto-competitive with the established toolkit: replay dominates the more complex A-GEM at
equal memory, and trades cleanly against EWC (no tuning and less forgetting, at the cost of a small
buffer). No exotic machinery is needed to make sequential onboarding safe.

\begin{table}[t]
\caption{Mitigation comparison at a matched $10\%$ memory budget (embedding-DeepLog, sequential
HDFS$\rightarrow$BGL, mean $\pm$ std, $n=3$). All three defenses remove the forgetting; our simple
replay matches the established A-GEM on retention/forgetting while learning the new domain better.
Sources: \texttt{results/mitigation\_\{replay,agem,ewc\}.csv}.}
\label{tab:baselines}
\centering
\begin{tabular}{@{}lcccc@{}}
\toprule
...
\midrule
No defense               & ---   & $0.039\pm0.003$ & $0.642\pm0.028$ & $+0.665\pm0.003$ \\
EWC ($\lambda{=}10^{8}$) & none  & $0.690\pm0.005$ & $\mathbf{0.752\pm0.052}$ & $+0.014\pm0.005$ \\
A-GEM~\cite{agem2019}    & $10\%$ & $0.698\pm0.002$ & $0.657\pm0.009$ & $+0.006\pm0.002$ \\
Replay (ours)            & $10\%$ & $\mathbf{0.700\pm0.004}$ & $0.722\pm0.052$ & $\mathbf{+0.004\pm0.004}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{The auditor's predictions are validated}\label{sec:validation}
An auditor is only useful if it is \emph{right}. We therefore run it \emph{blind} on every
configuration for which we have a measured outcome---two architectures, two input encodings, three
arrival patterns for the deep model, and the lightweight range-aggregation cases across the two- and
three-domain federations---and score its verdict against what actually happened, with the observed
outcomes read directly from the result files (Table~\ref{tab:validation};
\texttt{results/auditor\_validation.csv}). The auditor is correct in $14$ of $15$ configurations
($93\%$), including \emph{all ten} deep-model cells: it predicts exactly which (architecture, input,
arrival) combinations forget, having internalized that an attention model shares feature space even
under a scalar id. Its single error is a \emph{conservative false positive}: on OpenStack it flags the
range-aggregation coupling---which is structurally real, since OpenStack's tight length range is
contained in the global one---even though OpenStack's anomalies happen to be order-based, so no length
detector existed there to break. Erring toward flagging a real coupling is the safe direction for a
pre-deployment check, and no genuine failure is missed. The auditor is thus not a restatement of the
findings but a \emph{tested predictive instrument}.

\begin{table}[t]
\caption{The auditor, run blind, predicts the measured outcome in 14/15 configurations. Deep-model
cells span 2 architectures $\times$ 2 inputs $\times$ arrival patterns; lightweight cells are the
domain-blind Length collapse across 2- and 3-domain federations. Source:
\texttt{results/auditor\_validation.csv}.}
\label{tab:validation}
\centering
\begin{tabular}{@{}lcc@{}}
\toprule
Configuration group & Configs & Auditor correct \\
\midrule
...
in \emph{Proc. 7th Int. Conf. Learn. Represent. (ICLR)}, 2019.
